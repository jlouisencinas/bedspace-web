...
const cu   = n => Number(n || 0).toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
...
const periodLabel = (start, endExcl) => {
  const last = addDays(endExcl, -1)
  const [, sm, sd] = start.slice(0,10).split('-'); const [, em, ed] = last.split('-')
  return `${+sd} ${MO[+sm-1]} - ${+ed} ${MO[+em-1]}`
}

export default function PrintRentWater() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (c) setBillingDate(addDays(c.water_end, -1))
  ...
  const statements = useMemo(
    () => perTenant.filter(t => (t.rent + t.water + (t.addonRentWater || 0)) > 0.01),
    [perTenant]
  )
  ...
  const asOf   = addDays(cutoff.water_end, -1)
  const due    = billingDate ? addDays(billingDate, 7) : ''
  const period = periodLabel(cutoff.water_start, cutoff.water_end)

  const buildProps = (t, billNo, copyLabel) => {
    const room = roomMap[t.room_id] || {}
    const sp = splits.find(s => s.room_id === t.room_id && s.tenant_id === t.id && s.utility === 'WATER')
    const charges = [{ label: 'Monthly Rent', mid: period, amount: t.rent }]
    if (!t.special || t.water > 0) charges.push({ label: 'Add: Water (cu. m)', mid: cu(t.waterCons), amount: t.water })
    ...
    return {
      ...
      splitNote: sp ? `** custom split: ${Number(sp.weight_pct)}% of room water` : '',
    }
  }
    ...
    <div style={{ background: '#F0F4F8', minHeight: '100vh', padding: '16px 0' }}>
      <div className="no-print" style={{ maxWidth: 760, margin: '0 auto 16px', display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end', background: '#fff', padding: 14, borderRadius: 10, border: '1px solid #e2e8f0' }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 8, alignItems: 'center' }}>
          <span style={{ fontSize: 12, color: '#64748B' }}>{statements.length} bill(s) · Rent + Water</span>
